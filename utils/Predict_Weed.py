...
model_path = './models/RCNN_crop_weed_classification_model.h5'
svm_model_path = './models/svm_classifier.pkl'
def get_weed_model():
    global model
    global svm_model
    model = tf.keras.models.load_model(model_path)
    model = tf.keras.models.Model(model.inputs,model.layers[-5].output)
    with open(svm_model_path,'rb') as svm:
        svm_model = pickle.load(svm)

print(" * Loading Keras model for weed detector...")
get_weed_model()
...
def predict_weed(img_path):
    ...
    for index, rect in tqdm(enumerate(sel_rects)):
        ...
        feature = model.predict(resized_roi.reshape(-1, 224, 224, 3))
    ...
    for rect, score, cls in final:
        ...
        cv2.putText(imOut, cls + ':' + str(round(score * 100, 2)), (x, y - 8), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2,
                    cv2.LINE_AA)

    cv2.imwrite('prediction.jpeg', imOut)
    predicted = {'class': class_final, 'score': percentage}

    retval, buffer = cv2.imencode('.jpeg', imOut)

    base64_bytes = base64.b64encode(buffer)
    base64_str = base64_bytes.decode('utf-8')
    imaagedict = {'image': base64_str}

    decoded = base64.b64decode(base64_str)

    test_image = "testprediction.jpeg"
    with open(test_image, 'wb') as f:
        f.write(decoded)

    predicted.update(imaagedict)

    with open("weed-detection.json", "w") as outfile:
